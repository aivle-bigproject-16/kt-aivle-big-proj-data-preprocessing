# 배터리 이미지 데이터 전처리 파이프라인 v4.1

v4.1은 v4.0의 이미지 단위 대형 bbox 제외, Battery ID 단위 누수 방지, 결정론적 실행, YOLO detection/segmentation 변환과 승인 후 staging 승격 구조를 유지합니다. 변경점은 **CT ID 전체 보존**입니다. 이미지 제외율이나 annotation 밀도가 높더라도 Battery ID를 통째로 제외하지 않으며, 확인된 47 ID를 Test 7 + development 40으로 모두 배정합니다.

CT Test는 이미지 단위 pre-split 정책 이후 `defect_image_ratio`를 다섯 구간(zero, very_low, low_mid, mid_high, very_high)으로 나누고 largest-remainder로 구간별 quota를 계산해 강제합니다. quota는 ID 게이트를 적용하지 않은 47 ID에서 dry-run마다 다시 계산하며 과거 37-ID 실측값을 하드코딩하지 않습니다. 상세는 `데이터_전처리_v4.1_전체구현계획.md` §8.5, §9.4a와 §26.14에 있습니다.
...
→ 발견된 CT Battery ID 47개 전부 보존
→ 제외 후 Battery ID 통계 재계산(잔여 0장 ID 포함)
...
pwsh -File .\run_pipeline.ps1 -Stage dry-run `
  -RawRoot "<raw 데이터 루트>" -WorkDir "<v4.1 작업 폴더>" -Jobs 8 -Detached

pwsh -File .\run_pipeline.ps1 -Stage approve `
  -WorkDir "<v4.1 작업 폴더>" -ApprovedBy "<검토자>"

pwsh -File .\run_pipeline.ps1 -Stage execute `
  -RawRoot "<raw 데이터 루트>" -WorkDir "<v4.1 작업 폴더>" `
  -Output "<최종 출력 폴더>" -Jobs 8 -Detached

pwsh -File .\run_pipeline.ps1 -Stage upload `
  -WorkDir "<v4.1 작업 폴더>" -Output "<최종 출력 폴더>" `
...
```powershell
python -m compileall -q battery_v4_1 tests
python -m unittest discover -s tests -v
```

v3.6의 work-dir, 승인 파일 또는 ID CSV를 재사용하지 마십시오. v4.1 전용 작업 폴더에서 dry-run부터 다시 실행해야 합니다.
