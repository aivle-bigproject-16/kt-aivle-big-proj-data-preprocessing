# 배터리 이미지 데이터 전처리 파이프라인 v4.0

v4.0은 Battery ID 단위 누수 방지 분할, 결정론적 실행, YOLO detection/segmentation 변환과 승인 후 staging 승격 구조를 유지합니다. v3.x의 CT 대형 bbox 제외·ID 게이트·면적 층화에 더해, CT Test의 **ID 양성률 구간 quota를 하드 제약**으로 추가해 test가 특정 양성률 구간에 쏠리지 않게 합니다.

CT Test는 ID 양성률(pre-split·ID gate 이후 이미지의 defect_image_ratio)을 다섯 구간(zero, very_low, low_mid, mid_high, very_high)으로 나누고 largest-remainder로 구간별 quota를 계산해 강제합니다. 기존 축·양성률·밀도·면적 objective는 quota를 만족하는 조합 안에서만 최적화됩니다. 실데이터에서 quota는 2/2/1/1/1이며, 상세는 `데이터_전처리_v4.0_전체구현계획.md` §9.4a와 §26.14에 있습니다.
...
pwsh -File .\run_pipeline.ps1 -Stage dry-run `
  -RawRoot "<raw 데이터 루트>" -WorkDir "<v4.0 작업 폴더>" -Jobs 8 -Detached

pwsh -File .\run_pipeline.ps1 -Stage approve `
  -WorkDir "<v4.0 작업 폴더>" -ApprovedBy "<검토자>"

pwsh -File .\run_pipeline.ps1 -Stage execute `
  -RawRoot "<raw 데이터 루트>" -WorkDir "<v4.0 작업 폴더>" `
  -Output "<최종 출력 폴더>" -Jobs 8 -Detached

pwsh -File .\run_pipeline.ps1 -Stage upload `
  -WorkDir "<v4.0 작업 폴더>" -Output "<최종 출력 폴더>" `
...
v3.6의 work-dir, 승인 파일 또는 ID CSV를 재사용하지 마십시오. v4.0 전용 작업 폴더에서 dry-run부터 다시 실행해야 합니다.
