# 배터리 이미지 데이터 전처리 파이프라인 v3.7

v3.7은 Battery ID 단위 누수 방지 분할, 결정론적 실행, YOLO detection/segmentation 변환과 승인 후 staging 승격 구조를 유지하면서 CT 대형 bbox 제외 정책을 추가합니다.
...
v3.6의 work-dir, 승인 파일 또는 ID CSV를 재사용하지 마십시오. v3.7 전용 작업 폴더에서 dry-run부터 다시 실행해야 합니다.
